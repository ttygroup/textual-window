...
class WindowManager(DOMNode): 
    ...
    def reset_all_windows_position(self) -> None:
    ...
    def debug(self) -> None:
        ...
        self.log.debug(
            f"self.app: {self.app}"
            f"Layers: {self.app.screen.styles.layers}"
        )
    ...
    async def watch_for_dom_ready(self):
        ...
        try:
            ...
            raise NoActiveAppError(
                "No active app. Window manager has launched too early. Library bug."
            )
            ...
            self.reset_all_windows_position()   # Triggers the position calculation functions.   
